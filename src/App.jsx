...
const GOOGLE_SHEETS_SCRIPT_URL =
  'https://script.google.com/macros/s/AKfycbzJ_WHn3ssPL8VYbVbVOUa1Zw0xVFLolCnL-rOQ63cHO2st7KHqzZ9CHUwZhiCqVgBu/exec?v=getLeads';
const GOOGLE_SHEETS_USERS =
  'https://script.google.com/macros/s/AKfycbzJ_WHn3ssPL8VYbVbVOUa1Zw0xVFLolCnL-rOQ63cHO2st7KHqzZ9CHUwZhiCqVgBu/exec';
const GOOGLE_SHEETS_LEADS_FECHADOS =
...
const App = () => {
  ...
  const fetchLeadsFromSheet = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // Ordena por editado (mais recente primeiro)
  ...
  // ------------------- SINCRONIZAÇÃO CRIARLEAD -----------------
  const [leadsCriados, setLeadsCriados] = useState([]);

  useEffect(() => {
    const fetchLeadsCriadosFromSheet = async () => {
      try {
        const response = await fetch(GOOGLE_SHEETS_SCRIPT_URL);
        const data = await response.json();

        if (Array.isArray(data)) {
          const formattedLeadsCriados = data.map((item) => ({
            id: item.id || '',
            name: item.name || '',
            vehicleModel: item.vehiclemodel || '',
            vehicleYearModel: item.vehicleyearmodel || '',
          ...
          setLeadsCriados(formattedLeadsCriados);
        } else {
          setLeadsCriados([]);
        }
      } catch (error) {
        console.error('Erro ao buscar leads criados do Google Sheets:', error);
        setLeadsCriados([]);
      }
    };

    fetchLeadsCriadosFromSheet();
    const interval = setInterval(fetchLeadsCriadosFromSheet, 60000);
    return () => clearInterval(interval);
  }, []);
  ...
  const atualizarStatusLeadAntigo = (id, novoStatus, phone) => {
    ...
    setLeads((prev) =>
      prev.map((lead) => (lead.phone === phone ? { ...lead, status: novoStatus, confirmado: true } : lead))
    );
  };
  ...
  const atualizarSeguradoraLead = (id, seguradora) => {
    setLeads((prev) =>
      prev.map((lead) => (lead.id === id ? limparCamposLead({ ...lead, insurer: seguradora }) : lead))
    );
  };
  ...
  const confirmarSeguradoraLead = (id, premio, seguradora, comissao, parcelamento) => {
    ...
    setLeadsFechados((prev) => {
      const atualizados = prev.map((lead) => (lead.ID === id ? { ...lead, insurerConfirmed: true } : lead));
      return atualizados;
    });

    try {
      fetch(
        'https://script.google.com/macros/s/AKfycbzJ_WHn3ssPL8VYbVbVOUa1Zw0xVFLolCnL-rOQ63cHO2st7KHqzZ9CHUwZhiCqVgBu/exec?v=alterar_seguradora',
        {
          method: 'POST',
          mode: 'no-cors',
          body: JSON.stringify({
  ...
  const atualizarDetalhesLeadFechado = (id, campo, valor) => {
    setLeads((prev) => prev.map((lead) => (lead.id === id ? { ...lead, [campo]: valor } : lead)));
  };

  const transferirLead = (leadId, responsavelId) => {
    if (responsavelId === null) {
      setLeads((prev) => prev.map((lead) => (lead.id === leadId ? { ...lead, responsavel: null } : lead)));
      return;
    }
    ...
    setLeads((prev) => prev.map((lead) => (lead.id === leadId ? { ...lead, responsavel: usuario.nome } : lead)));
  };
  ...
  const atualizarStatusUsuario = (id, novoStatus = null, novoTipo = null) => {
    ...
    try {
      fetch(
        'https://script.google.com/macros/s/AKfycbzJ_WHn3ssPL8VYbVbVOUa1Zw0xVFLolCnL-rOQ63cHO2st7KHqzZ9CHUwZhiCqVgBu/exec?v=alterar_usuario',
        {
          method: 'POST',
          mode: 'no-cors',
          body: JSON.stringify({
      ...
      <main style={{ flex: 1, overflow: 'auto' }}>
        <Routes>
          ...
          <Route
            ...
            element={
              <Dashboard
                ...
                leadsPerdidos={
                  isAdmin ? leads : leads.filter((lead) => lead.responsavel === usuarioLogado.nome && lead.status === 'Perdido')
                }
                leads={isAdmin ? leads : leads.filter((lead) => lead.responsavel === usuarioLogado.nome && lead.status !== 'Fechado')}
                usuarios={usuarios}
              />
            }
          />
          ...
          <Route
            ...
            element={
              <LeadsPerdidos
                leadsPerdidos={isAdmin ? leads : leads.filter((lead) => lead.responsavel === usuarioLogado.nome && lead.status === 'Perdido')}
                atualizarStatusLead={atualizarStatusLead}
                usuarioLogado={usuarioLogado}
              />
            }
          ...
          <Route
            path="/buscar-lead"
            element={<BuscarLead leads={isAdmin ? leads : leads.filter((lead) => lead.responsavel === usuarioLogado.nome)} />}
          />
          ...
          <Route
            ...
            element={
              isAdmin ? (
                <Usuarios usuarios={usuarios} atualizarStatusUsuario={atualizarStatusUsuario} />
              ) : (
                <Navigate to="/" replace />
              )
            }
          ...
          <Route
            ...
            element={
              isAdmin ? (
                <CriarUsuario adicionarUsuario={adicionarUsuario} />
              ) : (
                <Navigate to="/" replace />
              )
            }
          ...
          {/* ROTA NOVA: Criar Lead */}
          <Route
            path="/criar-lead"
            element={
              <CriarLead
                adicionarLead={(novoLead) =>
                  setLeadsCriados((prev) => [...prev, { ...novoLead, id: prev.length + 1 }])
                }
                usuarios={usuarios}
                usuarioLogado={usuarioLogado}
              />
          ...
          <Route
            path="/ranking"
            element={<Ranking leadsFechados={leadsFechados} usuarioLogado={usuarioLogado} />}
          />
        </Routes>
      </main>
    </div>
